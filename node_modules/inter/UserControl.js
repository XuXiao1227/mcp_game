...
var Control = function () {
    ...
    var cmdArray = [
        ...
        {code: 'U09', fromType: keyFromType.CACHE, des: "任务列表"},
        {code: 'U10', fromType: keyFromType.CACHE, des: "用户兑换奖品"},
        {code: 'U11', fromType: keyFromType.CACHE, des: "奖品列表"},
        {code: 'U12', fromType: keyFromType.CACHE, des: "用户充值"},
        {code: 'U13', fromType: keyFromType.CACHE, des: "系统奖励"},
        {code: 'U14', fromType: keyFromType.CACHE, des: "更新用户信息"}
    ];
    self.cmd = {};
...
Control.prototype.checkU09 = function(headNode, bodyNode, cb){
    cb(null);
};

Control.prototype.checkU10 = function (headNode, bodyNode, cb){  //购买数据格式检测
    async.waterfall([
        function(cb)    //用户id和商品id必须存在
        {
            if(typeof bodyNode.itemId != 'number' ||
                typeof bodyNode.userId != 'number')
            {
                cb(ec[4000]);
            }
            else
        ...
        function(cb)    //商品是否可用
        {
            ...
            table.find({id:bodyNode.itemId}, {}, function(err, data){
                if(err || data.rst.length < 1)
                {
                    cb(ec[4001]);
                }
                ...
                {
                    var set = data.rst[0];
                    if(set.status != cons.goodsStatus.OPEN.id)
                    {
                        cb(ec[4002]);
                    }
                 //   else if (set.num < 1)
...
Control.prototype.checkU11 = function(headNode, bodyNode, cb){
    cb(null);
};

Control.prototype.checkU12 =  function(headNode, bodyNode, cb){
    cb(null);
};

Control.prototype.checkU13 = function(headNode, bodyNode, cb){
    async.waterfall([
        function(cb)    //gift type must exist
        {
            if(typeof bodyNode.giftType != 'number')
            {
                cb(ec[5000]);
            }
            else
        ...
        function(cb)    //check if the gift type is valid
        {
            switch(bodyNode.giftType){
                case cons.giftType.REGISTER.id:
                    //cb(null);
                    var table = dc.pg.get("gold_log");
                    var userId = parseInt(headNode.userId);
                    table.find({customer_id:userId, subject_id:2, amount:bodyNode.amount}, {}, function(err, data){
                        if(err || data.rst.length < 1){
                            cb(null);
                        }else{
                            cb(ec[5002]);
                    ...
                    break;
                case cons.giftType.LOGIN.id:
                    //cb(null);
                    var date = headNode.timeStamp.substring(0, 10);
                    var table = dc.pg.get("gold_log");
                    var userId = parseInt(headNode.userId);
                    var sql = "SELECT * FROM gold_log WHERE customer_id=" + userId + " AND subject_id=2 AND TO_CHAR(create_time, 'YYYY-MM-DD')='" + date + "'";
                    table.execute(sql, function(err, data){
                        if(err || data.rst.length < 1)
                        {
                            cb(null);
                        }else{
...
Control.prototype.checkU14 = function(headNode, bodyNode, cb){
    async.waterfall([
        function(cb)    //update option must exist
        {
            if(typeof bodyNode.option != 'string' )
            {
                cb(ec[1003]);
            }
            else
        ...
        function(cb)
        {
            switch(bodyNode.option){
                case "ROLLCHANGE":
                    if(typeof bodyNode.username != 'string' || bodyNode.password != 'string'){
                        cb(ec[1003]);
                    }else{
                        cb(null);
                    }
                    break;
                case "PASSWORD":
                    if(typeof bodyNode.password != 'string'){
                        cb(1003);
                    }else{
                        cb(null);
                    }
                    break;
                case "NICKNAME":
                    if(typeof bodyNode.nickname != 'string'){
                        cb(1003);
                    }else{
                        cb(null);
                    }
            ...
            if (ticket.game_code != "bonus"){
                ...
                table.update(cond, doc, {ret: ret}, function (err, data) {
                    if (data && data.rst.length > 0) {
                        ...
                        var ret = {gold: 1, date: 1};
                        var now = new Date().getTime();
                        pool.update(con, {$set: {date: dateUtil.toString(now)}}, function (err, data) {
                        });
                        pool.update(con, doc, {ret: ret}, function (err, data) {
                            if (data && data.rst.length > 0) {
                                backBody.goldPool = {gold: data.rst[0].gold, date: data.rst[0].date};
                                cb(null);
                            } else {
                                cb(err);
                            }
        ...
        function(cb)    //计算中奖金额
        {
            ...
            async.eachSeries(draw_info_array, function(info, callback) {
                ...
                table.find(cond, {hit_amount:1}, function(err, data){
                    if(data && data.rst.length > 0)
                    {
                        ticket.hit_amount += data.rst[0].hit_amount*ticket.multiple*info.count;
                        callback();
                    }
                    else
                    {
 ...
 * @param cb
 */
Control.prototype.handleU09 = function(headNode, bodyNode, cb){
    var backBody = {};
    async.waterfall([
        function(cb){
            var table = dc.pg.get("task");
            ...
            table.find(cons, cols, function(err, data){
                if(data){
                    backBody.rst = data.rst;
                }
                cb(err);
            });
        }
 ...
 * @param cb
 */
Control.prototype.handleU10 = function(headNode, bodyNode, cb){
    var backBody = {};
    var userId = parseInt(headNode.userId);
    var itemId = parseInt(bodyNode.itemId);
    async.waterfall([
        function(cb)    //扣款
        {
            var table = dc.pg.get("customer");
...
//        },
        function(cb)    //记录账户流水
        {
            var table = dc.pg.get("gold_log");
            var goldLog = {
                amount:(-1)*bodyNode.bill,
                before:backBody.customer.gold + bodyNode.bill,
                after:backBody.customer.gold,
                subject_id:cons.goldSubject.EXCHANGE.id,
                remark:cons.goldSubject.EXCHANGE.des + "item id: " + bodyNode.itemId,
                customer_id:userId,
                order_id:bodyNode.uuid
            };
            table.save(goldLog, function(err, data){
 ...
 * @param cb
 */
Control.prototype.handleU11 = function(headNode, bodyNode, cb){
    var backBody = {};
    async.waterfall([
        function(cb){
            var table = dc.pg.get("goods");
 ...
 * @param cb
 */
Control.prototype.handleU12 = function(headNode, bodyNode, cb){
    var backBody = {};
    var userId = parseInt(headNode.userId);
    async.waterfall([
        function(cb)
...
//        },
        function(cb)    //记录账户流水
        {
            ...
            var goldLog = {
                ...
                before:backBody.customer.gold - bodyNode.amount,
                after:backBody.customer.gold,
                subject_id:cons.goldSubject.CHARGE.id,
                remark:cons.goldSubject.CHARGE.des,
                customer_id:userId,
                order_id:bodyNode.uuid
            };
            table.save(goldLog, function(err, data){
 ...
 * @param cb
 */
Control.prototype.handleU13 = function(headNode, bodyNode, cb){
    var backBody = {};
    var userId = parseInt(headNode.userId);
    async.waterfall([
        function(cb)
...
//        },
        function(cb)    //记录账户流水
        {
            ...
            var goldLog = {
                ...
                after:backBody.customer.gold,
                subject_id:cons.goldSubject.SYSTEM_GIVE.id,
                remark:cons.goldSubject.SYSTEM_GIVE.des + ", 类型:" + bodyNode.giftType,
                customer_id:userId,
                order_id:bodyNode.uuid
            };
            table.save(goldLog, function(err, data){
 ...
 * @param cb
 */
Control.prototype.handleU14 = function(headNode, bodyNode, cb){
    var backBody = {};
    var userId = parseInt(headNode.userId);
    var table = dc.pg.get("customer");
    var cond = {id:userId};
    switch(bodyNode.option){
        case "ROLLCHANGE":
            if (typeof bodyNode.nickname != 'string') bodyNode.nickname="";
            var doc = {$set:{username:bodyNode.username, nickname:bodyNode.nickname, password:bodyNode.password, type:1}};
            table.update(cond, doc, function(err, data) {
                if (data && data.rst.length > 0) {
                    backBody.customer = {username: data.rst[0].username, password: data.rst[0].password, nickname: data.rst[0].nickname};
                    cb(null);
                }
                else {
                    cb(err);
            ...
            break;
        case "PASSWORD":
            table.update(cond, {$set:{password:bodyNode.password}}, function(err, data) {
                if (data && data.rst.length > 0) {
                    backBody.customer = {username: data.rst[0].username, password: data.rst[0].password};
                    cb(null);
                }
                else {
                    cb(err);
            ...
            break;
        case "NICKNAME":
            table.update(cond, {$set:{nickname:bodyNode.nickname}}, function(err, data) {
                if (data && data.rst.length > 0) {
                    backBody.customer = {username: data.rst[0].username, nickname: data.rst[0].nickname};
                    cb(null);
                }
                else {
                    cb(err);
