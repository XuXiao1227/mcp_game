 ...
 * @return VsString * 结构体指针
 */
VsString* vs_string_init(const char *src, long length) {
	return vs_string_init(src, length, VS_STRING_INIT_VOLUME);
}
...
struct VsStringList* vs_string_list_init() {
	struct VsStringList* list = (struct VsStringList*) malloc(
			sizeof(struct VsStringList));
	list->length = 0;
	list->volume = VS_STRING_LIST_INIT_VOLUME;
	list->pt = (struct VsString**) malloc(
			sizeof(struct VsString *) * list->volume);
	return list;
}
...
void vs_string_list_append(struct VsStringList* list, struct VsString* str) {
	...
	if (newLength > list->volume) {
		...
		list->pt = (struct VsString **) realloc(list->pt,
				sizeof(struct VsString*) * newVolume);
 ...
 *清空字符串，并设置成新的内容
 */
void vs_string_clear(VsString* str, const char *content) {
	*(str->pt) = '\0';
	str->length = 0;
	vs_string_append(str, content);
}

/**
 *克隆一个字符串
 */
VsString* vs_string_clone(VsString* str)
{
	return vs_string_init(str->pt, str->length);
}
